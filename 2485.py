'''
문제
직선으로 되어있는 도로의 한 편에 가로수가 임의의 간격으로 심어져있다. 가로수들이 모두 같은 간격이 되도록 가로수를 추가로 심어야 한다. 가능한 한 가장 적은 수의 나무를 심고 싶다. 가로수의 위치는 기준점으로 부터 떨어져 있는 거리로 표현되며, 위치는 모두 양의 정수이다.
...
for i in range(2, N):
  a.append(int(input()))
  nGCD = math.gcd(a[i]-a[i-1], nGCD)

cnt = 0
for i in range(1, N):
  cnt += int((a[i]-a[i-1])/nGCD)-1
