/*
문제
사탕가게에 설탕을 정확하게 N킬로그램을 배달해야 한다. 설탕공장에서 만드는 설탕은 봉지에 담겨져 있다. 봉지는 3킬로그램 봉지와 5킬로그램 봉지가 있다.
...
int main(){
    ...
    for(numFive=0;numFive<=quotient;numFive++){
        remain = N - numFive * 5;
        if(remain % 3 == 0){
            numThree = remain / 3;
            numTotal = numThree + numFive;
            if(numTotal < minTotal){
                minTotal = numTotal;
            }
        }
    }
