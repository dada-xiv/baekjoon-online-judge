...
cf) injective 함수는 P(M,N)이지만, 다리가 서로 겹쳐질 수 없기 때문에 조합인 C(M,N)으로 충분하다. 이때 나올 수 있는 최대의 수는 C(30,15)이며, 이는 2^31-1보다 작은 수이다. 그러나 int는 최대 12!를 처리할 수 있고 long long int는 최대 20!까지만 처리할 수 있다.
*/
